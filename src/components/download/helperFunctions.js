...
export const strainGenome = (dispatch, filePrefix, nodes, colorings, selectedNodesOnly, nodeVisibilities) => {
    ...
    /* traverse the tree & store tip information. We cannot write this out as we go as we don't know
       exactly which header fields we want until the tree has been traversed. */
    const tipGenomes = {};
    const headerFields = ["Strain", "Sequence"];
    const chars = ['A', 'C', 'T', 'G'];
    ...
    for (const [i, node] of nodes.entries()) {
        ...
        const dna = Array(200).fill(1).map(i => {return chars[Math.floor(Math.random()*chars.length)]}).join('');

        const wrapDNA = dna.match(/(.{1,60})/g).join("\n");

        tipGenomes[node.name] = {Strain: node.name};
        tipGenomes[node.name]["Sequence"] = wrapDNA;
    ...
    const linesToWrite = [];
    for (const [name, obj]  of Object.entries(tipGenomes)) {
        const head = ">" + name;
        const genome  = obj['Sequence'];
        linesToWrite.push(name + "\n" + genome);
    }

    /* write out information we've collected */
    const filename = `${filePrefix}${selectedNodesOnly ? "_selected_" : "_"}genomes.fasta`;
    write(filename, MIME.text, linesToWrite.join("\n"));
